...
6. 之后可以手动点“刷新行情并模拟成交”，也可以启动自动运行；自动运行支持 `Tick` 或 `Scan`，`Scan` 只生成待确认草案，连续错误熔断后可人工重置熔断状态。

前端会显示 BTC 实时价、24h 涨跌、行情延迟、上次 tick 状态、自动运行状态、运行前自检与处理建议、挂单距现价、风险摘要、草案冷却、最近风控事件、绩效统计、权益曲线、最近交易历史和账本备份列表，并每 15 秒自动刷新状态。点击“导出账本”可下载完整 paper JSON 账本，用于复盘或审计。若挂单距现价较远，模拟成交不会发生，这是策略等待反弹触发，不是行情失效。
...
- 自动运行启动前会执行 preflight，自检结果必须包含处理建议；自检失败时不得启动自动循环
- 自动运行 `scan` 模式只执行 `tick -> propose`，不会自动确认挂单
- 自动 tick 行情失败时只记录错误，不使用旧数据模拟交易
- 自动运行连续错误会熔断停止，阈值默认 3 次，可在启动自动运行时设置为 1-10 次
- `POST /api/auto/reset` 和前端“重置熔断”只清除本地自动运行错误/熔断状态，不启动自动运行、不生成草案、不修改 paper 账本
