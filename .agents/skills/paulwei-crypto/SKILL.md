...
6. 持续手动刷新行情并模拟成交，或启动自动运行；自动运行支持 `Tick` 或 `Scan`，`Scan` 只生成待确认草案，连续错误熔断后可人工重置熔断状态。

界面显示 BTC 实时价、24h 涨跌、行情延迟、上次 tick 状态、自动运行状态、运行前自检与处理建议、挂单距现价、风险摘要、草案冷却、最近风控事件、绩效统计、权益曲线、最近交易历史和账本备份列表，并每 15 秒自动刷新。点击“导出账本”可下载完整 paper JSON 账本，用于复盘或审计。若挂单离现价较远，说明策略在等待反弹到限价区，并不代表数据未更新。
...
- 自动运行启动前必须执行 preflight，自检结果必须包含处理建议；自检失败时不得启动自动循环。
...
- `POST /api/auto/reset` 和前端“重置熔断”只能清除本地自动运行错误/熔断状态，不得启动自动运行、生成草案或修改 paper 账本。
